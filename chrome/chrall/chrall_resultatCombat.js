	...
	chrall.analyseResultatCombat = function() {
		...
		for (var i = lines.length; i-- > 0;) {
			...
			if (line.indexOf("Résultat du Combat :") == 0) {
				...
				if (line.indexOf("Vous avez attaqué") == 0) { // quasiment tout est sur cette ligne, faut faire des acrobaties...
					...
					var damage = parseInt(line.substring(i0 - 4, i0).trim());
					if (damage && !isNaN(damage)) {
						result.Degats = damage;
					}
