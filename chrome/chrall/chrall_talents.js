(function (chrall) {
	...
	chrall.getBubbleContentForCompetence = function (name) {
		switch (name) {
			...
			case "Parer" :
				var parade = 3.5 * Math.floor(player.attac.diceNumber / 2) +
						Math.floor((player.attac.physicalBonus + player.attac.magicalBonus) / 2);
				var html = "<table>";
				html += "<tr><td>Jet de parade moyen</td><td> : " + parade + "</td></tr>";
				html += "<tr><td>\"Esquive\" moyenne totale</td><td> : " + (player.dodge.getMean() + parade) +
						"</td></tr>";
				html += "</table>";
				return html;
		...
		switch (name) {
			...
			case "Glue" :
				...
				html += "<tr><td>Portée (à l'horizontale uniquement)</td><td> : " +
						Math.floor(1 + player.totalSight / 3) + "</td></tr>";
				html += "</table>";
				return html;
			...
			case "Projectile Magique" :
				...
				for (d = 0; d <= range; d++) {
					...
					var degCrit = 2 * (Math.floor(player.sight.diceNumber * 0.75) + range - d) +
			...
			case "Téléportation" :
				...
				var f = function (mm) {
					...
					html += "<tr><td>Portée horizontale</td><td> : " + dh + "</td><td>&rArr; &nbsp; " +
							(player.x - dh) + " &le; X &le; " + (player.x + dh) + "</td><td>| &nbsp; " +
							(player.y - dh) + " &le; Y &le; " + (player.y + dh) + "</td></tr>";
					html += "<tr><td>Portée verticale</td><td> : " + dv + "</td><td>&rArr; &nbsp; " + (player.z - dv) +
			...
			case "Vampirisme" :
				...
				html += "<tr><td>Dégâts moyens</td><td> : " + deg + " / " + degCrit + "</td><td>(" +
						player.damage.diceNumber + "D3 " + degMagicalBonus + ")" + "</td></tr>";
				html += "</table>";
