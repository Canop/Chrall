...
func GetMessage(TrollId string, ChrallVersion string) (out *G2cMessage) {
	...
	currentChralVersion := &Version{[]uint{2, 2}} // il serait sans doute préférable de trouver un moyen d'avoir ça en "constante", ou bien une variable accrochée
	if userVersion, err := ParseVersion(ChrallVersion); err != nil {
		...
		case -1:
			...
			//out.Content += "<br>Il s'agit d'une version majeure qui introduit le partage d'informations entre joueurs."
