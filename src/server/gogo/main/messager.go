...
func GetMessage(TrollId string, ChrallVersion string) (out *G2cMessage) {
	...
	currentChralVersion := &Version{[]uint{1, 11}} // il serait sans doute préférable de trouver un moyen d'avoir ça en "constante", ou bien une variable accrochée
	if userVersion, err := ParseVersion(ChrallVersion); err != nil {
		...
		case -1:
			...
			out.Content += "<br>Cette version apporte le déplacement de la grille à la souris (essayez!) et le centrage au double-clic (essayez aussi!)."
