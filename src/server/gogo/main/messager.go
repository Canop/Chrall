...
func GetMessage(TrollId string, ChrallVersion string) (out *G2cMessage) {
	...
	currentChralVersion := &Version{[]uint{2, 20}} // il serait sans doute préférable de trouver un moyen d'avoir ça en "constante", ou bien une variable accrochée
	if userVersion, err := ParseVersion(ChrallVersion); err != nil {
		...
		case -1:
			...
			out.Content += "<br>Cette version corrige plusieurs petits bugs, la plupart liés aux évolutions récentes de MH, et en particulier l'envoi des CDM."
