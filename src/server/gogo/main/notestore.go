...
type Note struct {
	...
	Auteur    int
	TypeSujet string // rien, monstre, grotte ou troll
...
func (store *MysqlStore) SaveNote(db *mysql.Client, note *Note) (err os.Error) {
	if note.Id > 0 {
		...
		// insert
		sql := "insert into note (auteur, type_sujet, id_sujet, x_sujet, y_sujet, z_sujet, partage, date_changement, contenu)"
		sql += "          values (     ?,          ?,        ?,       ?,       ?,       ?,       ?,               ?,       ?)"
		...
		err = stmt.BindParams(note.Auteur, note.TypeSujet, note.IdSujet, note.XSujet, note.YSujet, note.ZSujet, note.Partage, seconds, note.Contenu)
		if err != nil {
