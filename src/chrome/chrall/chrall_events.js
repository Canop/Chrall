...
function Chrall_addBubblesToLinks() {
	$("a.mh_trolls_1").each(
		function() {
			...
			if (trollId!=player.id && trollId!=viewedTrollId) {
				bubble(link, '', "bub_troll", SERVEUR_CHRALL_PUBLIC+"json?action=get_troll_info&trollId="+trollId, trollId);
			}
		}
	);	
	...
	$("a.mh_monstres").each(
		function() {
			...
			bubble(link, '', "bub_monster", SERVEUR_CHRALL_PUBLIC+"json?action=get_extract_jsonp&name=" + encodeURIComponent(nomMonstre) + "&monsterId="+numMonstre, nomMonstre);
		}
	);
}
