...
func cdmValuesCell(min int, max int) string {
	if min == max {
		return "<td>" + strconv.FormatUint(uint64(min), 10) + "</td>"
	}
	if max == 0 {
		return "<td>" + strconv.FormatUint(uint64(min), 10) + " - +∞</td>"
	}
	return "<td>" + strconv.FormatUint(uint64(min), 10) + " - " + strconv.FormatUint(uint64(max), 10) + "</td>"
}
func addCdmValuesLine(c []string, label string, min int, max int) string {
	if min == 0 && max == 0 {
		return ""
	}
	return "<th>" + label + "</th>" + cdmValuesCell(min, max)
}
func addCdmValueLine(c []string, label string, v int) string {
	if v == 0 {
		return ""
	}
	return "<th>" + label + "</th><td>" + strconv.FormatUint(uint64(v), 10) + "</td>"
}
...
func addIntsNV(c []NameValue, label string, min int, max int) []NameValue {
	if min == 0 && max == 0 {
		return c
	}
	n := len(c)
	c = c[0 : n+1]
	if max == 0 {
		c[n] = NameValue{label, fmt.Sprint(min, " - ?")}
	} else if min == max {
		c[n] = NameValue{label, fmt.Sprint(min)}
	} else {
		c[n] = NameValue{label, fmt.Sprint(min, " - ", max)}
	}
	return c
}
func addIntNV(c []NameValue, label string, v int) []NameValue {
	if v == 0 {
		return c
	}
	n := len(c)
	c = c[0 : n+1]
	c[n] = NameValue{label, strconv.FormatUint(uint64(v), 10)}
	return c
}
...
func (cdm *CDM) HtmlTable() string {
	...
	lc = addIntsNV(lc, "Niveau", cdm.Niveau_min, cdm.Niveau_max)
	rc = addTextNV(rc, "Famille", cdm.Famille_text)
	lc = addIntsNV(lc, "Dés d'attaque", cdm.DésAttaque_min, cdm.DésAttaque_max)
	lc = addIntsNV(lc, "Dés d'esquive", cdm.DésEsquive_min, cdm.DésEsquive_max)
	lc = addIntsNV(lc, "Dés de dégâts", cdm.DésDégâts_min, cdm.DésDégâts_max)
	rc = addIntsNV(rc, "Points de vie", cdm.PointsDeVie_min, cdm.PointsDeVie_max)
	lc = addIntsNV(lc, "Dés de régénération", cdm.DésRégénération_min, cdm.DésRégénération_max)
	lc = addIntsNV(lc, "Armure", cdm.Armure_min, cdm.Armure_max)
	lc = addIntsNV(lc, "Armure physique", cdm.ArmurePhysique_min, cdm.ArmurePhysique_max)
	lc = addIntsNV(lc, "Armure magique", cdm.ArmureMagique_min, cdm.ArmureMagique_max)
	lc = addIntsNV(lc, "Vue", cdm.Vue_min, cdm.Vue_max)
	rc = addIntsNV(rc, "Maitrise magique", cdm.MaitriseMagique_min, cdm.MaitriseMagique_max)
	rc = addIntsNV(rc, "Résistance magique", cdm.RésistanceMagique_min, cdm.RésistanceMagique_max)
	bc = addTextNV(bc, "Capacité", cdm.Capacite_text)
	rc = addIntNV(rc, "Nombre d'attaques", cdm.NombreDAttaques)
	...
	rc = addBooleanNV(rc, "Vole", cdm.Vole_boolean)
	lc = addBooleanNV(lc, "Attaque à distance", cdm.AttaqueADistance_boolean)
	lc = addIntsNV(lc, "Durée du tour", cdm.DuréeTour_min, cdm.DuréeTour_max)
	rc = addTextNV(rc, "Portée du pouvoir", cdm.PortéeDuPouvoir_text)
	rc = addTextNV(rc, "Sang froid", cdm.SangFroid_text)
...
func (be *BestiaryExtract) Html(monsterId int, askerId int, m *TksManager, pourcentageBlessure int) (html string) {
...
func (tks *TrollInfos) HtmlTable(id int, m *TksManager) string {
	...
	guildStr := ""
	guilde := m.getGuildInfos(tks.IdGuilde)
	if guilde != nil {
